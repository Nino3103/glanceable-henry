...
import ChartCard, { ChartData } from "./ChartCard/ChartCard";
import ChartForm, { ChartFormData } from "./ChartForm/ChartForm";
  ...
  const handleFormSubmit = async (formData: ChartFormData) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/charts?chartType=${formData.chartType}&numericValue=${formData.numericValue}&metric=${formData.metric}&period=30d`
      );
      ...
      if (result.success && result.data) {
        ...
        const labels =
          categories[formData.metric as keyof typeof categories] ||
          categories.category;
        ...
        chartData = labels.slice(0, dataCount).map((label) => ({
          ...
          value:
            Math.floor(
              Math.random() * (formData.metric === "revenue" ? 50000 : 1000)
            ) + (formData.metric === "revenue" ? 10000 : 100),
        }));
      }
      ...
      {charts.length === 0 ? (
        <div className="bg-muted border border-border rounded-lg p-8 text-center">
          <p className="text-muted-foreground">
            No charts added yet. Click &quot;Add Chart&quot; to create your
            first chart.
          </p>
        </div>
      ) : (
